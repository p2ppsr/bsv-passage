...
- Recovery: the exact signed raw transaction is atomically checkpointed before broadcast. An uncertain request resolves that transaction ID first and can only resend identical bytes. A BRC-100 migration with an ambiguous `signAction` result is never retried automatically.
